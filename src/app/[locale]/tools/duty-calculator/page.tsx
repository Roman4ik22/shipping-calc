  ...
  const features: { icon: React.ReactNode; color: string; tintBg: string; title: string; tag: string; body: string; stat: string }[] = [
    {
      ...
      title: t(loc, "feat_hs_title") || "Full duty + VAT breakdown",
      tag: t(loc, "feat_hs_tag") || "Not just the duty rate",
      body: t(loc, "feat_hs_body") || "Declared value → customs duty → import VAT → broker handling → total landed cost. Every line itemized so you can explain the invoice to finance.",
      stat: t(loc, "feat_hs_stat") || "4 line items per result",
    },
    {
      ...
      title: t(loc, "feat_refresh_title") || "Published tariffs, updated weekly",
      tag: t(loc, "feat_refresh_tag") || "Sourced directly",
      body: t(loc, "feat_refresh_body") || "Rates come from each country's published tariff schedule — HTSUS, TARIC, UK Integrated Online Tariff, CBSA. No scraping, no guesswork, updated on a weekly cadence.",
      stat: t(loc, "feat_refresh_stat") || "Weekly refresh, 213 authorities",
    },
    {
      ...
      title: t(loc, "feat_dm_title") || "De minimis thresholds",
      tag: t(loc, "feat_dm_tag") || "Low-value exemptions",
      body: t(loc, "feat_dm_body") || "$800 US · €150 EU · £135 UK · CAD 40 · AUD 1,000. We track the threshold and the conditions — your shipment only qualifies when the full set of rules is met.",
      stat: t(loc, "feat_dm_stat") || "213 thresholds tracked",
    },
    {
      ...
      title: t(loc, "feat_honest_title") || "Free and honest",
      tag: t(loc, "feat_honest_tag") || "No signup, no upsell",
      body: t(loc, "feat_honest_body") || "The calculator is and stays free. No carrier commissions means the answer we give you is the one our data actually says — not the one that pays us most.",
      stat: t(loc, "feat_honest_stat") || "Zero paywalled features",
    },
  ];
    ...
    <>
        ...
        <div style={{ position: "relative", maxWidth: 1240, margin: "0 auto", padding: "0 32px" }}>
          ...
          <div style={{ maxWidth: 820, marginBottom: 44 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 12px 6px 8px", borderRadius: 999, background: "#fff", border: "1px solid var(--line)", fontSize: 12, fontWeight: 600, color: "var(--ink-2)", boxShadow: "var(--shadow-sm)" }}>
              ...
              {t(loc, "duty_hero_pill") || "213 countries · free · updated weekly"}
            </div>
            <h1 style={{ margin: "22px 0 18px", fontSize: "clamp(40px,5.5vw,72px)", lineHeight: 1.02, letterSpacing: "-.03em", fontWeight: 800, color: "var(--ink)" }}>
              {t(loc, "duty_hero_h1_pre") || "Know exact duties"}<br />
              <span style={{ color: "var(--accent)" }}>{t(loc, "duty_hero_h1_accent") || "before"}</span> {t(loc, "duty_hero_h1_post") || "you ship — not after."}
            </h1>
            <p style={{ fontSize: 19, color: "var(--body)", lineHeight: 1.55, maxWidth: 640, margin: 0 }}>
              {t(loc, "duty_hero_desc") || "Landed-cost math in seconds. Duty, VAT/GST, broker fees, de minimis thresholds — applied to your destination country and declared value, for every territory we cover."}
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "96px 32px", background: "#fff", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ marginBottom: 40, maxWidth: 720 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "features_eyebrow") || "What's inside"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, color: "var(--ink)" }}>
              {t(loc, "features_title") || "Four things the generic calculators miss."}
            </h2>
            <p style={{ margin: "14px 0 0", fontSize: 17, color: "var(--body)", maxWidth: 620 }}>
              {t(loc, "features_desc") || "Most \"duty calculators\" online are a single-country lookup bolted to a currency converter. We cover the full landed-cost stack, for every market."}
            </p>
          </div>
      ...
      <section style={{ padding: "96px 32px" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }} className="two-col">
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--accent)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "coverage_eyebrow") || "213 countries & territories"}</div>
              <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, color: "var(--ink)" }}>
                {t(loc, "coverage_title") || "Every destination your customers actually ship to."}
              </h2>
              <p style={{ fontSize: 17, color: "var(--body)", lineHeight: 1.55, margin: "18px 0 28px", maxWidth: 500 }}>
                {t(loc, "coverage_desc") || "Not just the G7. From Andorra to Zimbabwe — duty rates, VAT/GST, de minimis thresholds and customs formalities indexed directly from each authority's published tariff schedule."}
              </p>
              ...
              <div style={{ background: "var(--bg)", borderRadius: 18, border: "1px solid var(--line)", padding: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "var(--muted)" }}>{t(loc, "sample_profile") || "Sample country profile · weekly update"}</div>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                  {[
                    [t(loc, "duty_range") || "Duty range", "0–17%", t(loc, "ad_valorem") || "Ad valorem"],
                    [t(loc, "vat_gst") || "VAT / GST", "19%", t(loc, "on_dutiable_base") || "On dutiable base"],
                    [t(loc, "de_minimis") || "De minimis", "€150", t(loc, "value_threshold") || "Value threshold"],
                  ].map(([l, v, s]) => (
              ...
              <Link href={`/${locale}/customs/united-states`} style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 24, fontSize: 14, fontWeight: 700, color: "var(--blue)", textDecoration: "none" }}>
                {t(loc, "browse_all_profiles") || "Browse all country profiles"}
      ...
      <section style={{ padding: "96px 32px", background: "#fff", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--good)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "comparison_eyebrow") || "Side by side"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", color: "var(--ink)" }}>
              {t(loc, "comparison_title") || "Built for commerce. Not for filling out a textbook exercise."}
            </h2>
          </div>
          <div style={{ background: "var(--bg)", borderRadius: 20, border: "1px solid var(--line)", overflow: "hidden", boxShadow: "var(--shadow-md)" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr", background: "var(--ink)", color: "#fff" }}>
              <div style={{ padding: "22px 24px", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".1em", color: "rgba(255,255,255,.5)" }}>{t(loc, "capability") || "Capability"}</div>
              ...
              <div style={{ padding: "22px 20px", textAlign: "center", fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,.6)" }}>{t(loc, "carrier_builtins") || "Carrier built-ins"}</div>
              <div style={{ padding: "22px 20px", textAlign: "center", fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,.6)" }}>{t(loc, "generic_calculators") || "Generic calculators"}</div>
            </div>
            {(() => {
              const rows: [string, boolean | string, boolean | string, boolean | string][] = [
                [t(loc, "cmp_row_duty_vat") || "Duty + VAT + broker fees", true, "Partial", "Duty only"],
                [t(loc, "cmp_row_213") || "213 countries covered", true, "Partial", false],
                [t(loc, "cmp_row_dm") || "De minimis threshold logic", true, false, false],
                [t(loc, "cmp_row_weekly") || "Weekly refresh", true, "Monthly", "Unknown"],
                [t(loc, "cmp_row_landed") || "Full landed-cost breakdown", true, false, false],
                [t(loc, "cmp_row_12lang") || "12 languages", true, false, "English only"],
                [t(loc, "cmp_row_signup") || "No signup required", true, false, true],
                [t(loc, "cmp_row_price") || "Price", t(loc, "cmp_free") || "Free", t(loc, "cmp_bundled") || "Bundled with label", t(loc, "cmp_free") || "Free"],
              ];
      ...
      <section style={{ padding: "64px 32px", background: "var(--ink)", color: "#fff" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 32 }} className="stats-grid">
            {[
              ["213", t(loc, "stat_countries_label") || "Countries covered"],
              ["12", t(loc, "stat_langs_label") || "Languages"],
              [t(loc, "stat_weekly_v") || "Weekly", t(loc, "stat_weekly_l") || "Data refresh cadence"],
              [t(loc, "stat_free_v") || "Free", t(loc, "stat_free_l") || "Forever · no signup"],
            ].map(([v, l]) => (
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "auto 1fr", gap: 28, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 800, color: "#A37A00", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>
              {t(loc, "disclaimer_eyebrow") || "Good-faith estimate — not a binding determination"}
            </div>
            <p style={{ margin: 0, fontSize: 14, color: "var(--ink-2)", lineHeight: 1.55, maxWidth: 900 }}>
              {t(loc, "disclaimer_body") || "Final duty is assessed by the destination customs authority at time of clearance and may differ based on inspection, valuation method, or origin documentation. Our numbers aim to be close to what you'll actually pay — but this is not legal or customs advice. For shipments above $25,000 or regulated goods, consult a licensed customs broker."}
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "96px 32px", background: "#fff" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "faq_eyebrow") || "Questions we get weekly"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", color: "var(--ink)" }}>
              {t(loc, "faq_title") || "How the math actually works."}
            </h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[
              {
                q: t(loc, "duty_faq_1_q") || "Where do your duty rates come from?",
                a: t(loc, "duty_faq_1_a") || "Directly from each country's published tariff schedule — the US HTSUS, the EU TARIC database, the UK Integrated Online Tariff, Canada's CBSA tariff, and equivalents for all other markets. We do not scrape carrier quote engines.",
              },
              {
                q: t(loc, "duty_faq_2_q") || "How often is the data updated?",
                a: t(loc, "duty_faq_2_a") || "Weekly. Major tariff changes (e.g. new US/EU sanctions) are pulled through out-of-band, but the normal cadence is a weekly refresh from each authority's published feed.",
              },
              {
                q: t(loc, "duty_faq_3_q") || "Does the calculator account for trade agreements?",
                a: t(loc, "duty_faq_3_a") || "For the major agreements (USMCA, EU-UK TCA, CPTPP, Mercosur) we apply the preferential rate in the country profile where your origin qualifies. Rule-of-origin documentation is your responsibility — we do not generate certificates of origin.",
              },
              {
                q: t(loc, "duty_faq_4_q") || "What if the customs authority charges something different than your estimate?",
                a: t(loc, "duty_faq_4_a") || "Final assessment is always the customs authority's call. Our number is a good-faith estimate based on the published tariff schedule. Differences most often come from valuation method, HS classification disputes, or unexpected regulatory findings (licensing, restricted goods). For shipments above $25,000 or regulated categories, please use a licensed broker.",
              },
              {
                q: t(loc, "duty_faq_5_q") || "Do you handle regulated goods?",
                a: t(loc, "duty_faq_5_a") || "We flag general restrictions for lithium batteries, cosmetics, alcohol, pharmaceuticals and other regulated categories in the country profiles. Actual licensing and permits are handled outside the calculator — typically through a destination-country customs broker.",
              },
              {
                q: t(loc, "duty_faq_6_q") || "Is the calculator really free?",
                a: t(loc, "duty_faq_6_a") || "Yes. No signup, no paywall, no rate-limit on lookups. We're a small team in Hungary (Global Supply KFT) and the calculator will stay free.",
              },
            ].map((it, i) => (
          ...
          <script
            ...
            dangerouslySetInnerHTML={{
              __html: JSON.stringify({
                ...
                "@type": "FAQPage",
                mainEntity: [
                  { "@type": "Question", name: t(loc, "duty_faq_1_q") || "Where do your duty rates come from?", acceptedAnswer: { "@type": "Answer", text: t(loc, "duty_faq_1_a") || "Directly from each country's published tariff schedule." } },
                  { "@type": "Question", name: t(loc, "duty_faq_2_q") || "How often is the data updated?", acceptedAnswer: { "@type": "Answer", text: t(loc, "duty_faq_2_a") || "Weekly." } },
                ],
              }),
            }}
          />
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", borderRadius: 28, background: "var(--ink)", color: "#fff", padding: "72px 48px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", maxWidth: 720 }}>
            <h2 style={{ margin: 0, fontSize: "clamp(32px,4vw,52px)", fontWeight: 800, letterSpacing: "-.025em", lineHeight: 1.05 }}>
              {t(loc, "duty_cta_pre") || "Your customers shouldn't learn about customs at their"}{" "}
              <span style={{ color: "var(--warm)" }}>{t(loc, "duty_cta_accent") || "front door."}</span>
            </h2>
            <p style={{ fontSize: 17, color: "rgba(255,255,255,.7)", lineHeight: 1.55, margin: "20px 0 32px", maxWidth: 520 }}>
              {t(loc, "duty_cta_desc") || "Estimate the all-in price before you ship. No sign-up. No payment. No surprise invoices when the parcel clears customs."}
            </p>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              <a href="#" style={{ padding: "14px 22px", borderRadius: 12, background: "var(--warm)", color: "var(--ink)", fontSize: 15, fontWeight: 800, display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none" }}>
                {t(loc, "duty_cta_primary") || "Run another quote"}
              ...
              <Link href={`/${locale}/guide`} style={{ padding: "14px 22px", borderRadius: 12, background: "rgba(255,255,255,.08)", color: "#fff", fontSize: 15, fontWeight: 700, border: "1px solid rgba(255,255,255,.14)", textDecoration: "none" }}>
                {t(loc, "duty_cta_secondary") || "Browse country guides"}
              </Link>
            </div>
          </div>
        </div>
