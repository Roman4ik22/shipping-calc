  ...
  return (
    <>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1240, margin: "0 auto", padding: "96px 32px 112px" }}>
          ...
          <div style={{ maxWidth: 820 }}>
            ...
            <p style={{ fontSize: 20, color: "var(--body)", lineHeight: 1.55, maxWidth: 680, margin: 0 }}>
              {t(loc, "about_hero_lead") || "Fuel surcharges that change mid-week. Dim-weight formulas nobody explains. Customs bills that arrive after the package. We're rebuilding that layer — transparently, neutrally, with published data."}
            </p>
            <div style={{ marginTop: 36, display: "flex", gap: 28, flexWrap: "wrap", fontSize: 13, color: "var(--muted)" }}>
              {[
                [t(loc, "stat_langs_short") || "Languages", "12"],
                [t(loc, "stat_countries_short") || "Countries", "213"],
                [t(loc, "stat_routes_short") || "Routes", "45,000+"],
                [t(loc, "stat_carriers_short") || "Carriers", "134+"],
              ].map(([k, v]) => (
                <div key={k}>
      ...
      <section style={{ padding: "96px 32px", position: "relative" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ marginBottom: 40, maxWidth: 720 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "origin_eyebrow") || "Origin story"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, color: "var(--ink)" }}>{t(loc, "origin_title") || "Why we built this."}</h2>
          </div>
            ...
            <div style={{ position: "relative" }}>
              <div style={{ background: "#fff", borderRadius: 20, border: "1px solid var(--line)", padding: 28, boxShadow: "var(--shadow-lg)", transform: "rotate(2deg)", maxWidth: 440, marginLeft: "auto", position: "relative" }}>
                ...
                <div style={{ position: "absolute", transform: "rotate(-12deg)", right: 24, bottom: 60, border: "3px solid var(--accent)", color: "var(--accent)", padding: "6px 12px", borderRadius: 6, fontSize: 11, fontWeight: 800, letterSpacing: ".08em", opacity: 0.85 }}>
                  {t(loc, "unexpected_stamp") || "UNEXPECTED"}
                </div>
              </div>
      ...
      <section style={{ padding: "80px 32px", background: "#fff", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ marginBottom: 40, maxWidth: 720 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "mission_eyebrow") || "Mission"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, color: "var(--ink)" }}>{t(loc, "mission_title") || "Three principles. Non-negotiable."}</h2>
            <p style={{ margin: "14px 0 0", fontSize: 17, color: "var(--body)", maxWidth: 620 }}>
              {t(loc, "mission_desc") || "Everything we build — rate tables, customs calculator, country guides — gets measured against these three."}
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="tools-grid">
            {[
              { icon: <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>, tint: "var(--blue)", tintBg: "var(--blue-50)", title: t(loc, "principle_transparency") || "Transparency", tag: t(loc, "principle_transparency_tag") || "Every fee upfront", desc: t(loc, "principle_transparency_desc") || "No hidden surcharges. Fuel, remote-area, dim-weight and customs fees shown before you ship, not when the package lands at a border." },
              { icon: <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v18M5 8h14M3 14l3-7 3 7M15 14l3-7 3 7" /></svg>, tint: "var(--accent)", tintBg: "var(--accent-50)", title: t(loc, "principle_neutrality") || "Neutrality", tag: t(loc, "principle_neutrality_tag") || "No carrier commissions", desc: t(loc, "principle_neutrality_desc") || "We take zero rebates from DHL, FedEx or anyone else. Ranking on the results page is the honest one — cheapest wins, not highest-kickback." },
              { icon: <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><circle cx="12" cy="12" r="5" /><circle cx="12" cy="12" r="1.5" fill="currentColor" /></svg>, tint: "#A37A00", tintBg: "var(--warm-50)", title: t(loc, "principle_accuracy") || "Accuracy", tag: t(loc, "principle_accuracy_tag") || "Published tariffs only", desc: t(loc, "principle_accuracy_desc") || "Rates sourced from carrier tariff documents, updated weekly. Customs data from official authority databases across 213 countries." },
            ].map((c, i) => (
      ...
      <section style={{ padding: "96px 32px" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ maxWidth: 620, marginBottom: 40 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "team_eyebrow") || "Team"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, color: "var(--ink)" }}>{t(loc, "team_about_title") || "A small team, moving fast."}</h2>
            <p style={{ margin: "14px 0 0", fontSize: 17, color: "var(--body)" }}>
              {t(loc, "team_about_desc") || "We're a small team based in Hungary. No big offices — just people who think shipping pricing should be transparent."}
            </p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }} className="tools-grid">
            {team.map((m) => (
      ...
      <section style={{ padding: "80px 32px", background: "#fff", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ marginBottom: 40, maxWidth: 720 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 12 }}>{t(loc, "timeline_eyebrow") || "Timeline"}</div>
            <h2 style={{ margin: 0, fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, color: "var(--ink)" }}>{t(loc, "timeline_title") || "From idea to 134 carriers in 3 months."}</h2>
          </div>
          <div style={{ overflowX: "auto", paddingBottom: 24 }}>
            <div style={{ display: "flex", gap: 0, padding: "48px 0 32px", minWidth: "max-content", position: "relative" }}>
              ...
              {[
                { y: "Jan '26", t: t(loc, "tl_jan_t") || "The idea", d: t(loc, "tl_jan_d") || "Spent an afternoon comparing 10 carrier websites for one parcel. Decided there had to be a better way.", badge: null },
                { y: "Feb '26", t: t(loc, "tl_feb_t") || "First prototype", d: t(loc, "tl_feb_d") || "Built a working rate aggregator across 40 carriers and 50 countries. Tested with real shipments from Hungary.", badge: null },
                { y: "Mar '26", t: t(loc, "tl_mar_t") || "Public launch", d: t(loc, "tl_mar_d") || "RateShips goes live with 134 carriers across 213 countries. Customs duty calculator and delivery estimator included from day one.", badge: "Launch" },
                { y: "Apr '26", t: t(loc, "tl_apr_t") || "12 languages", d: t(loc, "tl_apr_d") || "Added full translation for Russian, Spanish, German, French, Portuguese, Chinese, Japanese, Korean, Arabic, Turkish, and Italian.", badge: null },
                { y: "Q2 '26", t: t(loc, "tl_q2_t") || "45,000+ routes", d: t(loc, "tl_q2_d") || "Expanded corridor coverage with weekly rate updates. Every major trade lane gets at least 3 carrier options.", badge: null },
                { y: "Q3 '26", t: t(loc, "tl_q3_t") || "Tools & guides", d: t(loc, "tl_q3_d") || "Country-specific shipping guides for 195+ destinations. HS-code lookup integrated into customs calculator.", badge: "Planned" },
                { y: "Q4 '26", t: t(loc, "tl_q4_t") || "API for platforms", d: t(loc, "tl_q4_d") || "Read-only API for e-commerce platforms. Shopify and WooCommerce plugins in development.", badge: "Planned" },
              ].map((e) => {
          ...
          <div style={{ fontSize: 12, color: "var(--muted)" }}>
            {t(loc, "timeline_hint") || "← scroll horizontally to see the full timeline →"}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "80px 32px", background: "var(--ink)", color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 1240, margin: "0 auto" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "var(--warm)", textTransform: "uppercase", letterSpacing: ".1em", marginBottom: 10 }}>{t(loc, "by_numbers") || "By the numbers"}</div>
          <h2 style={{ margin: "0 0 40px", fontSize: "clamp(28px,3.2vw,40px)", fontWeight: 800, letterSpacing: "-.02em", lineHeight: 1.1, maxWidth: 720 }}>
            {t(loc, "company_stats_title") || "A small company with big receipts."}
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 32 }} className="stats-grid">
            {[
              { v: "2026", l: t(loc, "founded") || "Founded", s: "Hungary", accent: false },
              { v: "134+", l: t(loc, "stat_carriers_label") || "Carriers", s: t(loc, "carriers_published_tariffs") || "published tariffs", accent: false },
              { v: "213", l: t(loc, "stat_countries_label") || "Countries", s: t(loc, "every_territory") || "every territory", accent: false },
              { v: "12", l: t(loc, "stat_langs_label") || "Languages", s: t(loc, "updated_weekly") || "updated weekly", accent: true },
            ].map((it, i) => (
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", padding: "80px 32px 96px" }}>
        ...
        <section style={{ marginTop: 48, padding: "32px 28px", background: "#fff", border: "1px solid var(--line)", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--blue)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>{t(loc, "contact") || "Contact"}</div>
            <h3 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "var(--ink)" }}>{t(loc, "about_contact_heading") || "Questions? We reply to everything."}</h3>
          </div>
