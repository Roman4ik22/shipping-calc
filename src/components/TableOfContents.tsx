...
export default function TableOfContents({
  ...
  return (
    <nav className="mb-8 bg-white rounded-2xl p-5" aria-label="Table of contents">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {items.map((item) => (
          <button
            ...
            className="px-3 py-1.5 bg-[#F8F5EF] rounded-full text-body hover:text-ink transition-colors cursor-pointer"
          >
            {item.label}
          </button>
        ))}
        <Link
          ...
          className="px-3 py-1.5 bg-[#F8F5EF] rounded-full text-body hover:text-ink transition-colors"
        >
